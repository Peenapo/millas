    ...
    */
    ...
    carousel: {
        ...
        start: function() {
            ...
            if( ! $('.bw-carousel').length ) { return; }

            App.carousel.setImage();
            App.carousel.bind();
        ...
        setImage: function() {
            ...
            $('.bw-carousel-slide').each(function() {
        ...
        bind: function() {
            ...
            var $firstSlide = $('.bw-carousel-slide:eq(0)');

            $('.bw-carousel').imagesLoaded(function() {
                if( $firstSlide.hasClass('bw-call-readability') ) {
                    $body.addClass('bw-readability bw-header-readability');
                }
            });

            $('.bw-carousel').on('click', '.bw-carousel-nav', App.carousel.click)
        ...
        click: function(e) {
            ...
            if( $('.bw-carousel').hasClass('bw-animate') ) { return; }

            var self = $(this);

            var dir = self.hasClass('bw-carousel-nav-next');

            var nextIndex = App.carousel.index + ( dir ? +1 : -1 );

            if( nextIndex >= $('.bw-carousel-slide').length ) { nextIndex = 0; }
            if( nextIndex < 0 ) { nextIndex = $('.bw-carousel-slide').length - 1; }

            App.carousel.slide( App.carousel.index, nextIndex, dir );

            App.carousel.index = nextIndex;

        },

        slide: function( currentIndex, nextIndex, dir ) {

            $('.bw-carousel').addClass('bw-animate bw-hide-title');

            setTimeout(function() {
                $('.bw-carousel').removeClass('bw-hide-title');
            }, 500 );

            var $currentSlide = $('.bw-carousel-slide').eq( currentIndex );
            var $nextSlide = $('.bw-carousel-slide').eq( nextIndex );

            if( $nextSlide.hasClass('bw-call-readability') ) {
                setTimeout(function() {
                    $body.addClass('bw-readability bw-header-readability');
                }, 300);
            }else{
                $body.removeClass('bw-readability bw-header-readability');
            }

            $('.bw-carousel-slide').removeAttr('style');
            ...
            TweenLite.fromTo( $nextSlide, .8, {x: ( dir ? '100%' : '-100%' ) }, {x:'0%', ease: Power2.easeInOut, onComplete: function() {
                $('.bw-carousel').removeClass('bw-animate');
            }});
