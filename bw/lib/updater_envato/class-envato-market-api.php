...
if ( ! class_exists( 'Bw_Envato_Market_API' ) ) :
/**
 * Creates the Envato API connection.
 *
 * @class Bw_Envato_Market_API
 * @version 1.0.0
 * @since 1.0.0
 */
class Bw_Envato_Market_API {
	 ...
	 *
	 * @see Bw_Envato_Market_API()
	 * @uses Bw_Envato_Market_API::init_globals() Setup class globals.
	 * @uses Bw_Envato_Market_API::init_actions() Setup hooks and actions.
	 *
	 * @since 1.0.0
	 * @static
	...
	private function init_globals() {
		$this->strings = Bw_theme_updater::get_strings();
		$this->token = Bw_guide::get_envato_updater_token();
	}
	...
	public function can_make_request_with_token() {
		...
		if ( ( $can = get_transient( 'bw_updater_can_make_request' ) ) == false ) {
			$can = $this->item( '11830820' ) ? true : false;
			set_transient( 'bw_updater_can_make_request', $can, HOUR_IN_SECONDS );
		}
		return $can;
	}
