<?php
if ( ! class_exists( 'Bw_Envato_Market_Updater' ) ) :
/**
 * Shims in our premium automatic updates alongside the standard
 * WordPress.org updates.
 *
 * @package Bw_Envato_Market_Updater
 */
class Bw_Envato_Market_Updater {
	...
	*/
	...
	private static function init_actions() {
		add_filter( 'site_transient_update_themes', array( 'Bw_Envato_Market_Updater', 'check_theme_updates' ) );
		add_filter( 'delete_site_transient_update_themes', array( 'Bw_Envato_Market_Updater', 'delete_theme_update_transient' ) );
		add_action( 'load-update-core.php', array( 'Bw_Envato_Market_Updater', 'delete_theme_update_transient' ) );
		add_action( 'load-themes.php', array( 'Bw_Envato_Market_Updater', 'delete_theme_update_transient' ) );

		// Deferred Download.
		add_action( 'upgrader_package_options', array( 'Bw_Envato_Market_Updater', 'maybe_deferred_download' ), 99 );
	}
	...
	*
	*     @type string $package                     Required path to the package source.
	*     @type string $destination                 Required path to a folder to install the package in.
	*     @type bool   $clear_destination           Whether to delete any files already in the destination folder.
	*     @type bool   $clear_working               Whether to delete the files form the working directory after copying to the destination.
...
Bw_Envato_Market_Updater::instance();
